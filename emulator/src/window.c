...
void window_init() {
    ...
    registers =
        newwin(window_rows / 2 - 4, window_cols / 2, 4, window_cols / 2);
...
void windows_status(char* status) { wprintw(serial, "%s", status); }
...
void window_lcd_home() { wmove(lcd, 1, 1); }

void window_lcd_putc(char ch) { wechochar(lcd, ch); }

void window_serial_putc(char ch) {
    if (ch != '\r') wechochar(serial, ch);
}

void window_set_mem() {
    char memloc[32];
    int height = 5, width = 30;     // Dialog box dimensions
    int start_y = 5, start_x = 10;  // Dialog box position

    WINDOW* dialog_box = newwin(height, width, start_y, start_x);
    box(dialog_box, 0, 0);
    mvwprintw(dialog_box, 1, 1, "Enter memory addres in hex:");
    wrefresh(dialog_box);
    echo();
    mvwgetstr(dialog_box, 2, 1, memloc);
    noecho();
    uint32_t newmemloc = strtol(memloc, 0, 16);
    if (newmemloc < 0x10000) memory_address = newmemloc;
    delwin(dialog_box);
    touchwin(memory);
    wrefresh(memory);
}

void window_shutdown() { endwin(); }
...
void window_mem_forward() {
    memory_address += 0x32;
    if (memory_address > 0xFFFF) memory_address -= 0x32;
}

void window_mem_backward() {
    memory_address -= 0x32;
    if (memory_address < 0) memory_address = 0;
}
